...
const ACTIVE_DOWNLOAD_STATES = ['queued', 'downloading', 'paused', 'waiting_network'];
...
export default function Layout({ children, title, back }: { children: ReactNode; title?: string; back?: string }) {
  ...
  return (
    <div className="min-h-dvh bg-[#0a0a0c] text-zinc-100">
      {/* ---- desktop sidebar ---- */}
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-60 flex-col border-r border-white/[0.06] bg-[#0c0c0f] lg:flex">
        <div className="flex items-center gap-2.5 px-5 pb-2 pt-6">
          <IconLogo size={24} />
          <span className="text-[15px] font-semibold tracking-tight">Video Vault</span>
        </div>
        <nav className="mt-4 flex flex-col gap-0.5 px-3">
          <SideLink to="/" end icon={<IconLibrary size={17} />} label="Library" />
          <SideLink to="/transfers" icon={<IconTransfers size={17} />} label="Transfers" badge={activeCount || undefined} />
          {user?.role === 'admin' && <SideLink to="/admin" icon={<IconUsers size={17} />} label="Users" />}
        </nav>
        <div className="mt-auto space-y-0.5 px-3 pb-5">
          <button
            onClick={() => setWhatsNewOpen(true)}
            className="flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-[13px] text-zinc-500 transition-colors hover:bg-white/[0.05] hover:text-zinc-200"
          >
            <IconSparkle size={15} /> What's new · v{APP_VERSION}
          </button>
          <button
            onClick={() => void runUpdateCheck()}
            className="flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2 text-[13px] text-zinc-500 transition-colors hover:bg-white/[0.05] hover:text-zinc-200"
          >
            <IconDownload size={15} /> {updateState === 'idle' ? 'Check for updates' : updateSub}
          </button>
          <div className="mt-2 flex items-center gap-2.5 border-t border-white/[0.06] px-2.5 pt-3.5">
            <span className="flex h-8 w-8 items-center justify-center rounded-full border border-white/10 bg-white/[0.05] text-[12px] font-semibold text-zinc-300">
              {user?.username.slice(0, 1).toUpperCase()}
            </span>
            <span className="min-w-0 flex-1">
              <span className="block truncate text-[13px] font-medium">{user?.username}</span>
              <span className="block text-[11px] text-zinc-600">{user?.role === 'admin' ? 'Administrator' : 'Member'}</span>
            </span>
            <button
              onClick={() => void logout()}
              className="flex h-8 w-8 items-center justify-center rounded-full text-zinc-600 transition-colors hover:bg-white/[0.06] hover:text-zinc-200"
              aria-label="Sign out"
            >
              <IconLogout size={16} />
            </button>
      ...
      <div className="flex min-h-dvh flex-col lg:pl-60">
        <header className="sticky top-0 z-30 border-b border-white/[0.06] bg-[#0a0a0c]/95 pt-[env(safe-area-inset-top)] backdrop-blur">
          <div className="mx-auto flex h-[52px] w-full max-w-lg items-center gap-2.5 px-4 lg:max-w-6xl lg:px-8">
            {back !== undefined ? (
              <button
              ...
              <span className="text-zinc-100 lg:hidden">
                <IconLogo size={22} />
              </span>
            )}
            <h1 className="min-w-0 flex-1 truncate text-[16px] font-semibold tracking-tight lg:text-[18px]">
              {title ?? 'Video Vault'}
            </h1>
            <button
              onClick={() => setMenuOpen(true)}
              className="flex h-8 w-8 items-center justify-center rounded-full border border-white/10 bg-white/[0.05] text-[12px] font-semibold text-zinc-300 transition-colors hover:bg-white/10 lg:hidden"
              aria-label="Account"
            >
              {user?.username.slice(0, 1).toUpperCase()}
            </button>
        ...
        <main className="mx-auto w-full max-w-lg flex-1 px-4 pb-28 pt-4 lg:max-w-6xl lg:px-8 lg:pb-12 lg:pt-6">
          {needsFile.length > 0 && (
            <div className="mb-4 rounded-xl border border-amber-500/25 bg-amber-500/[0.07] p-4">
              <p className="text-[13px] font-semibold text-amber-200">
                {needsFile.length} upload{needsFile.length === 1 ? ' is' : 's are'} waiting for{' '}
        ...
        {/* ---- mobile bottom nav ---- */}
        <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-white/[0.06] bg-[#0e0e11]/95 pb-[env(safe-area-inset-bottom)] backdrop-blur lg:hidden">
      ...
      <Sheet open={menuOpen} onClose={() => setMenuOpen(false)}>
        ...
        <div className="space-y-0.5">
          ...
          />
          <SheetAction icon={<IconDownload size={18} />} label="Check for updates" sub={updateSub} onClick={() => void runUpdateCheck()} />
...
function SideLink({ to, icon, label, badge, end }: { to: string; icon: ReactNode; label: string; badge?: number; end?: boolean }) {
  return (
    <NavLink
      to={to}
      end={end}
      className={({ isActive }) =>
        `flex items-center gap-2.5 rounded-lg px-2.5 py-2 text-[13.5px] font-medium transition-colors ${
          isActive ? 'bg-white/[0.07] text-zinc-100' : 'text-zinc-500 hover:bg-white/[0.04] hover:text-zinc-200'
        }`
      }
    >
      {icon}
